...
route.post("/signin", async (req, res) => {
  ...
  console.log(body);
  console.log("Querying Collection:", StudentModel.collection.name);
    ...
    .then((user) => {
      ...
      if (user) {
        if (user.role === role.Student && user.past?.status === true) {
          return res.json({ error: "Wrong Password or  ID 1" });
        } else if (user.role === role.Student && user.withdraw === true) {
          return res.json({ error: "Wrong Password or  ID 2" });
        } else if (req.body.password == user.password) {
          console.log(req.body.password == user.password);
          return res.json({ success: true, user });
        } else {
          return res.json({ error: "Wrong Password or  ID 3" });
        }
      } else {
        return res.json({ error: "Wrong Password or  ID 4" });
      }
    })
    .catch((err) => {
      console.log(err);
